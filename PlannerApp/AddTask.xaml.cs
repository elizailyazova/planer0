...
namespace PlannerApp
{
    public partial class AddTask : Window
    {
        ...
        public AddTask()
        {
            ...
            SaveCommand = new RelayCommand(_ => Add(null, null));
        ...
        private void Add(object sender, RoutedEventArgs e)
        {
            ...
            {
                System.Windows.MessageBox.Show($"Ошибка при создании задачи: {ex.Message}");
            }
        }
        ...
        private void OpenTask(TaskItem task)
        {
            if (task.Id > 0)
            {
                ...
                {
                    task = db.GetTaskById(task.Id); // Используем новый метод
                }
            }

            var window = new Tasks(task) { Owner = this };
        ...
        private void SaveTaskChanges(TaskItem task)
        {
            using (var db = new DatabaseHelper())
            {
                // Используем GetTaskById для получения задачи по Id
                var existingTask = db.GetTaskById(task.Id);
                ...
                if (existingTask != null)
                {
                    ...
                    // Нужно будет добавить метод для сохранения изменений в базе
                    db.SaveChanges(existingTask);
                }
            }
        }
        ...
        private void LoadTasks()
        {
            try
            {
                using (var db = new DatabaseHelper())
                {
                    // Получаем все задачи через db.GetTasks()
